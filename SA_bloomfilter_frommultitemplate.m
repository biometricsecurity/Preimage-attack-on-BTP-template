function SA_bloomfilter_frommultitemplate(bitss,starti,endi)
...
for j=starti:endi
    ...
    attack_ids((j-1)*3+1):attack_ids((j-1)*3+3)
    f_fitness = @(x)fitness_bloomfilter_twohashcode(x,bf_templates( attack_ids((j-1)*3+1):attack_ids((j-1)*3+3),:),opts); % fitness function
...
save(['data/bloomfilter/bloomfilter_reconstructnoconstraint',num2str(starti),'_',num2str(endi),'_',num2str(opts.N_BITS_BF),'.mat'],'reconstruct_x');
